// src/pages/members/BusinessMembers.tsx
...
const BusinessMembers: React.FC = () => {
  ...
  // Helper function to translate business type
  const translateBusinessType = (type: string): string => {
    const typeMap: Record<string, string> = {
      '도매': intl.formatMessage({ id: 'pages.members.business.businessTypeWholesale' }),
      '소매': intl.formatMessage({ id: 'pages.members.business.businessTypeRetail' }),
      '수입': intl.formatMessage({ id: 'pages.members.business.businessTypeImport' }),
      '수출': intl.formatMessage({ id: 'pages.members.business.businessTypeExport' }),
    };
  ...
  const generateMockData = () => {
    ...
    return Array.from({ length: 42 }, (_, i) => ({
      ...
      representative: ['김대표', '이사장', '박대표', '최사장', '정대표'][
        Math.floor(Math.random() * 5)
      ],
      email: `business${1000 + i}@company.com`,
      ...
      totalAmount: Math.floor(Math.random() * 2000) * 10000,
      unpaidAmount:
        Math.random() > 0.85 ? Math.floor(Math.random() * 100) * 10000 : 0,
      ...
      lastLogin: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000)
        ...
        .split('T')[0],
      joinedAt: new Date(
        Date.now() - Math.random() * 1200 * 24 * 60 * 60 * 1000,
      )
        .toISOString()
        .split('T')[0],
      memo: Math.random() > 0.9 ? '사업자 회원, 대량 구매' : null,
      isBlack: false,
      memberType: '사업자',
      businessType: ['도매', '소매', '수입', '수출'][
  ...
  const columns: ProColumns<any>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'pages.members.business.memberType' }),
      dataIndex: 'memberType',
      width: 100,
      hideInSearch: true,
      render: () => <Tag color="green">{intl.formatMessage({ id: 'pages.members.business.memberTypeTag' })}</Tag>,
    },
    {
      title: intl.formatMessage({ id: 'pages.members.business.businessNumber' }),
    ...
    {
      title: intl.formatMessage({ id: 'pages.members.business.representative' }),
      dataIndex: 'representative',
      width: 100,
    },
    {
      title: intl.formatMessage({ id: 'pages.members.business.businessType' }),
      dataIndex: 'businessType',
      width: 100,
      valueType: 'select',
      valueEnum: {
        도매: { text: intl.formatMessage({ id: 'pages.members.business.businessTypeWholesale' }), status: 'Processing' },
        소매: { text: intl.formatMessage({ id: 'pages.members.business.businessTypeRetail' }), status: 'Success' },
        수입: { text: intl.formatMessage({ id: 'pages.members.business.businessTypeImport' }), status: 'Default' },
        수출: { text: intl.formatMessage({ id: 'pages.members.business.businessTypeExport' }), status: 'Warning' },
      },
      render: (_, record) => <Tag color="blue">{translateBusinessType(record.businessType)}</Tag>,
    },
    {
  ...
  return (
    <PageContainer title={intl.formatMessage({ id: 'pages.members.business.title' })}>
      {/* Statistics */}
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col span={6}>
          <Card>
            <Statistic
              title={intl.formatMessage({ id: 'pages.members.business.totalBusiness' })}
              value={4111}
              prefix={<ShopOutlined />}
            />
          </Card>
            ...
            items={[
              {
                ...
                children: (
                  <Descriptions bordered column={2}>
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.business.memberType' })}>
                      <Tag color="green">{intl.formatMessage({ id: 'pages.members.business.memberTypeLabel' })}</Tag>
                    </Descriptions.Item>
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.list.memberId' })}>
                      {selectedMember.userId}
                    </Descriptions.Item>
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.business.companyName' })}>
                      {selectedMember.name}
                    </Descriptions.Item>
                    ...
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.business.businessNumber' })} span={2}>
                      <Space>
                        <FileTextOutlined />
                        <Text copyable>{selectedMember.businessNumber}</Text>
                      </Space>
                    </Descriptions.Item>
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.business.representative' })}>
                      {selectedMember.representative}
                    </Descriptions.Item>
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.business.businessType' })}>
                      <Tag color="blue">{translateBusinessType(selectedMember.businessType)}</Tag>
                    </Descriptions.Item>
              ...
              },
              {
                ...
                children: (
                  <Descriptions bordered column={2}>
                    ...
                    <Descriptions.Item label={intl.formatMessage({ id: 'pages.members.business.monthlyAverage' })} span={2}>
                      {Math.floor(selectedMember.orderCount / 12)}{intl.formatMessage({ id: 'pages.members.list.count' })}
                    </Descriptions.Item>
                  </Descriptions>
                ),
              },
