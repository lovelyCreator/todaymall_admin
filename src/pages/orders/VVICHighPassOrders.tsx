// src/pages/orders/VVICHighPassOrders.tsx
...
const VVICHighPassOrders: React.FC = () => {
  const intl = useIntl();
  ...
  const [selectedLabel, setSelectedLabel] = useState<string>(intl.formatMessage({ id: 'pages.orders.vvicHighpass.title' }));
  ...
  const getSelectedCount = (statusGroups: any[]): number => {
    if (!selectedCode) return 125000; // VVIC HighPass orders total
    const found = statusGroups.flatMap((g) => g.items).find(
      (i: any) => i.code === selectedCode,
    );
    return found ? Math.floor(found.count * 0.25) : 125000; // 25% for VVIC HighPass orders
  };
  ...
  const translateOrderType = (type: string): string => {
    const typeMap: Record<string, string> = {
      ...
      'VVIC 하이패스': intl.formatMessage({ id: 'pages.orders.status.vvicHighpass' }),
  ...
  const handleClearFilter = () => {
    ...
    setSelectedLabel(intl.formatMessage({ id: 'pages.orders.vvicHighpass.title' }));
    setSelectedRowKeys([]);
  ...
  return (
    <>
      ...
      <PageContainer
        title={
          <Space>
            {intl.formatMessage({ id: 'pages.orders.vvicHighpass.title' })}
        ...
        <ProTable
          ...
          request={async (params) => {
            ...
            const mockData = Array.from(
              ...
              (_, i) => {
                const orderNo = `VVIC20251121${String(10000 + start + i).padStart(5, '0')}`;
...
export default VVICHighPassOrders;
