// src/pages/orders/UserOrders.tsx
...
const UserOrders: React.FC = () => {
  ...
  const [selectedLabel, setSelectedLabel] = useState<string>(intl.formatMessage({ id: 'pages.orders.user.title' }));
  ...
  const getSelectedCount = (statusGroups: any[]): number => {
    if (!selectedCode) return 285430; // User orders total
    const found = statusGroups.flatMap((g) => g.items).find(
      (i: any) => i.code === selectedCode,
    );
    return found ? Math.floor(found.count * 0.65) : 285430; // 65% for user orders
  };
  ...
  const handleClearFilter = () => {
    ...
    setSelectedLabel(intl.formatMessage({ id: 'pages.orders.user.title' }));
    setSelectedRowKeys([]);
  ...
  return (
    <>
      ...
      <PageContainer
        title={
          <Space>
            {intl.formatMessage({ id: 'pages.orders.user.title' })}
        ...
        <ProTable
          ...
          request={async (params) => {
            ...
            const mockData = Array.from(
              ...
              (_, i) => {
                const orderNo = `USR20251121${String(10000 + start + i).padStart(5, '0')}`;
